...
# project
from kl3m_embeddings.stats.svds import get_layer_svd_stats
...
DEFAULT_TOTAL_STEPS: int = 100000
...
class KL3MTorchTrainer(abc.ABC):
    ...
    def get_trailing_loss(self, steps: int = 10) -> float:
        ...
        losses = [
            loss
            for loss in self.loss_ts[-steps:]
    ...
    def backward(self, loss: torch.Tensor, **kwargs) -> None:
        ...
        # scale loss by gradient accumulation steps and world size
        gradient_accumulation_steps = self.training_config.get("optimizer", {}).get(
            "gradient_accumulation_steps", 1
        )
        scaled_loss = loss / self.world_size / math.sqrt(gradient_accumulation_steps)
    ...
    def save(self) -> None:
        ...
        try:
            ...
            if "safe_serialization=False" in str(e):
                ...
                self.log("Error saving model: %s", str(e), level="error")
    ...
    def eval(self) -> dict[str, Any]:
        ...
        with torch.no_grad():
            ...
            # get stats
            try:
                stddev = statistics.stdev(sample_loss)
            except:
                stddev = 0.0
            eval_stats = {
    ...
    def train(self, steps: Optional[int] = None) -> bool:
        ...
        # write eval data for restart/post-mortem
        eval_file_path = self.checkpoint_path / "eval_samples.jsonl"
        with eval_file_path.open("wt", encoding="utf-8") as eval_file:
            for sample in self.eval_samples:
                # convert tensor to list
                output_sample = sample.copy()
                for key in output_sample:
                    if isinstance(output_sample[key], torch.Tensor):
                        output_sample[key] = output_sample[key].tolist()
                eval_file.write(json.dumps(output_sample) + "\n")
        ...
        with torch.amp.autocast(device_type=self.device, dtype=self.precision):
            # run an initial eval to make sure we compiled
            self.eval()
            ...
            try:
                ...
                while step < (steps or total_steps):
                    ...
                    # check for nan loss with torch
                    if torch.isnan(outputs.loss) and step > 100:
                        self.log("NaN loss detected, skipping step", level="warning")
                    else:
